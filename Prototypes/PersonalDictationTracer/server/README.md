...
The default production stack sends OTLP/HTTP protobuf metrics, logs, and traces to its pinned Collector. The Collector exposes only Prometheus metrics on the internal Compose network; a pinned Prometheus instance retains them for 14 days or 2 GB, whichever limit is reached first. Prometheus is the only monitoring port published on Ronin, and it is bound to `127.0.0.1:9090`.

```bash
make server-latency
```

The report queries fixed recording rules for the latest 24-hour request count, HTTP p50/p95/p99 by recording-length bucket, service p95, recognition real-time-factor p50/p95/p99, and every bounded stage's p50/p95/p99. Non-finite quantiles are omitted, so an empty array means the stack is healthy but has no qualifying sample in the window. From another personal machine, use an SSH or Tailscale SSH local-forward to Ronin rather than publishing port 9090 to the tailnet. The Prometheus UI can then be opened through that local tunnel.

Collector or Prometheus failure never delays or fails dictation: the Effect exporter batches off the request path, applies a circuit breaker after export failures, and resumes later. Compose bounds both monitoring containers, runs them without root or Linux capabilities, and persists only Prometheus metrics in `hex-prometheus-data`. The default Collector deliberately discards exported trace and log payloads after accepting them; the service's bounded container JSON logs remain available through `make server-logs`. Add a private trace or log backend only through a separately reviewed Collector change.

The principal latency series are `hex_http_server_duration_ms`, `hex_dictation_stage_duration_ms`, `hex_dictation_service_duration_ms`, and `hex_dictation_recognition_realtime_factor`. The real-time factor is recognition time divided by recording duration, so values below `1` mean inference is faster than playback. HTTP, service, and real-time-factor histograms use a bounded recording-length bucket, while immutable build, runtime, and model revisions are resource attributes. A per-deployment `service.instance.id` is the non-secret rotated process epoch used to prove the replacement process exported telemetry; recording rules deliberately aggregate it away. This separates a genuine regression from a different recording workload or artifact revision. Request counts are bounded by route, method, outcome, status class, replay state, and recording-length class. Audio byte and duration histograms measure workload shape without recording audio or transcript content.
...
Record the first physical-phone stop-to-insert p50/p95/p99 baseline before declaring a performance regression threshold. The committed rules alert when either the Collector scrape or application export disappears, but intentionally contain no guessed latency threshold. Deployment requires Prometheus to observe the exact new service revision before that revision is considered promoted. The iOS client already measures capture stop, upload, server timing, mailbox delivery, and insertion; the server stage histograms identify which portion can actually be made faster. Do not substitute a generous CI wall-clock assertion for that real-device baseline.

This implements the server-retention portion of issue #8, not the complete performance policy. The issue remains open until physical-phone observations use the specified monotonic stop-to-insert endpoints, those observations are reported idempotently, Ronin resource isolation is measured, and an approved versioned baseline supplies minimum sample counts and numerical regression budgets.
